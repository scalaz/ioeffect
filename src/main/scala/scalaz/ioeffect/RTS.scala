...
private object RTS {
  ...
  final case class Finalizer[E](finalizer: ExitResult[E, Any] => IO[E, Unit])
  ...
  final class FiberContext[E, A](rts: RTS,
      ...
      while (!caught && !stack.isEmpty()) {
        stack.pop() match {
          ...
          case f0: Finalizer[_] =>
            ...
            val currentFinalizer: IO[E2, List[E]] =
              f.finalizer(body).attempt[E2].map {
              ...
              if (opcount == maxopcount) {
                ...
                (curIo.tag: @switch) match {
                  ...
                  case IO.Tags.AsyncIOEffect =>
                    ...
                    try {
                      val value =
                        rts.tryUnsafePerformIO(io.register(resumeAsync))
                  ...
                  case IO.Tags.Bracket =>
                    ...
                    curIo = for {
                      ...
                      b <- io.use(a)
                      _ <- (io.release(ExitResult.Completed(b), a) <* IO.sync(
                            ref.set(null)
                          )).uninterruptibly
                    } yield b
    ...
    private final def raceWith[A, B, C](
      ...
      IO.flatten(IO.async0[E, IO[E, C]] { resume =>
        ...
        def callback[A1, B1](
          other: Fiber[E, B1],
          finish: (A1, Fiber[E, B1]) => IO[E, C]
        ): ExitResult[E, A1] => Unit =
    ...
    final def exitSupervision(e: Throwable): IO[E, Unit] =
      IO.flatten(IO.sync {
        supervising -= 1
  ...
  def SuccessUnit[E]: ExitResult[E, Unit] =
